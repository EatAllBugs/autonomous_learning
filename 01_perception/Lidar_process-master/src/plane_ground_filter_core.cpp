...
void PlaneGroundFilter::CropBox_filer(pcl::PointCloud<VPoint>::Ptr in, pcl::PointCloud<VPoint> &out)
{
    pcl::CropBox<VPoint> region(true);
    region.setMin(Eigen::Vector4f (0, -3, -0.6, 1));
    region.setMax(Eigen::Vector4f (80, 8, 2, 1));
...
void PlaneGroundFilter::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
    ...
    // 1.Msg to pointcloud
    pcl::PointCloud<VPoint> laserCloudIn_;
    pcl::PointCloud<VPoint> laserCloudIn;

    pcl::fromROSMsg(*in_cloud_ptr, laserCloudIn_);
    CropBox_filer(laserCloudIn_.makeShared(),laserCloudIn);

    ROS_INFO("lidarCloudIn_ size %d",laserCloudIn_.size());
    ROS_INFO("lidarCloudIn size %d",laserCloudIn.size());
